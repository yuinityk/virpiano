...
def createSawtoothWave (A, freqList, fs, length):
    data = []
    amp = float(A) / len(freqList)
    # [-1.0, 1.0]の小数値が入った波を作成
    for n in arange(length * fs):  # nはサンプルインデックス
        s = 0.0
        for k in range(0, 15): #サンプルごとに10個のサイン波を重ねあわせ
       		for f in freqList:
       			s += amp * np.sin(2 * np.pi * k * f * n / fs)
        # 振幅が大きい時はクリッピング
        if s > 1.0:  s = 1.0
        if s < -1.0: s = -1.0
        data.append(s)
...
if __name__ == "__main__" :
    ...
    while(n<11):
    	...
    	for freqList in chordList:
    		data = createSawtoothWave(1.0, freqList, 8000.0, 1.0)
    		play(data, 8000, 16)
       	n=n+1 
